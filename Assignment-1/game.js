...
// Function to simulate sleep for animtions
function sleep(ms) {
...
async function startAnimation() {
    for (let i = 0; i < 20; i++) { 
        /* Roll the first die and adjust for array indexing (0-based) */
        die1Value = rollDice() - 1;
        /* Roll the second die and adjust for array indexing (0-based) */
        die2Value = rollDie() - 1;
        /* Update the dice images in the UI */
        updateImages();
        /* Pause for the 100 milliseconds to create an animation effect */
        await sleep(100);
    }  
...
function processResult(sum) {
    ...
    if (point === 0)
        ...
        if (sum === 7 || sum === 11) {
        document.getElementById("lblResult").innerHTML = "Congratulatioins! You win!";
        document.getElementById("btnPlayAgain").disabled = true;
        document.getElementById("btnRollDice").disabled = false;
}
//Check if the sum of the dice is 2, 3, or 12 (Craps)
else if (sum === 2 || sum === 3 || sum === 12) {
    document.getElementById("lblResult").innerHTML = "Sorry, you suck! Better luck next time!";
...
// Subsequent rolls
    ...
    if (sum === point) {
        ...
        document.getElementById("btPlayAgain").disabled = false;
        document.getElementById("btnRollDice").disabled = true;
}
    // Check if the sum of the dice is 7
    else if (sum === 7) {
        document.getElementById("lblResult").innerHTML = "Sorry, you rolled a 7! You suck!";
        point = 0; // Reset point for the next game
        document.getElementById("btnPlayAgain").disable = false;
        document.getElementById("btnRolld dice").disabled = true;
    }
...
function playAgain() {
    ...
    // Enable the 'Roll Dice' button for the player to inititiate a new roll
    document.getElementById("btnRollDice").disabled = false;

    // Reset the background images of both dice to the inititial image
